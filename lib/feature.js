    ...
    new Feature(file)
      ...
      .on('step', function(keyword, content, line) {
        ...
        var tokens = content.match(step.regex),
          bindArgs = [last.ctx].concat(tokens.slice(1)),
          handler = step.fn && step.fn.bind.apply(step.fn, bindArgs);

        last.addTest(new Test(keyword + content, handler));
      })
      .scan();
  });
...
feature.printMissing = function printMissing(missings) {
  ...
  missings.forEach(function(missing, i) {
    ...
    keyword = /^And/.test(keyword) ? feature.findPrev(missings, i).keyword : keyword;

    console.log([
      keyword.trim() + '(' + feature.makeRegExp(regex) + ', function() {',
...
feature.makeRegExp = function makeRegExp(path, sensitive) {
  if (path instanceof RegExp) return path;
  path = path.replace(/([\/.])/g, '\\$1');
  return new RegExp(path, sensitive ? '' : 'i');
};

// Find the very first occurence of a previous step that's not an And keyword,
// and returns it.
feature.findPrev = function findPrev(array, index) {
  var match, item;
  for(index; !match; index--) {
    item = array[index];
    if(item && item.keyword.trim() !== 'And') match = true;
  }
