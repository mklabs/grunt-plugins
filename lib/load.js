...
plugin.attach = function(options) {
  ...
  grunt.load = function(dir) {
    ...
    files = files.filter(function(file) {
      return !!~exts.indexOf(extname(file));
    });
...
plugin.require = function _require(modpath) {
  ...
  try {
    ...
    // fallback to vm trickery, returns the raw string, `plugin.require` will
    // do the rest

    // got compiled to js it it happens to be CoffeeScript
    code = fs.readFileSync(modpath, 'utf8')
    return extname(modpath) === '.coffee' ? cs.compile(code) : code;
  }
};
...
plugin.context = function context(filename, grunt, sb) {
  ...
  sb.require = sb.require || function (p) {
    var rel = p.match(/^\.\.?\//);
    return require(rel ? resolve(dirname, p) : p);
  };
